...
#+attr_latex: :font \footnotesize :float margin :placement [72pt]
| First | Second | Third |
|-------+--------+-------|
| One   | Two    | Three |
| Six   | Seven  | Eight |
...
* Tufte Org Mode Manual                                            :noexport:
:PROPERTIES:
:EXPORT_FILE_NAME: tufte-org-mode
...
Tufte Org mode defines several Org mode macros for convenience.
 ...
 - sidenote :: This macro takes three arguments:
   ...
   - an placement expressed as a LaTeX length, where positive values move
...
has an =placement= option that lets you move the citation up or down as you
...
the figure in the margin can be adjusted up or down using the =:placement=
...
,#+caption[Helix in the margin]: Helix in the margin.
,#+caption: Note that this figure fits in the margin.
,#+attr_latex: :float margin :width 2in :placement -2.5in 
[[file:helix.pdf]]
#+end_example
...
=:float margin=.  In addition, there are attributes to: set the placement,
e.g., =:placement 1in=, which adjusts the position of the figure up or
...
command takes an optional parameter for the placement, and this is
implemented as a LaTeX attribute associated with the Org mode footnote
definition, e.g.,
#+begin_example
#+attr_latex: :placement 36pt
